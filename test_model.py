...
def test(data_dir, csv_path, splits_path, output_dir, target='pa',
    ...
    if misc.test_only_pa:
        ...
        savepreds['y_preds_pa_only'] = y_preds_pa_only
        savepreds['meta_pa_only'] = row

        test_metrics_df = test_metrics_df.append(row, ignore_index=True)
        test_metrics_df.to_csv(resultsfile, index=False)

        saveauc['auc_pa_only'] = per_label_auc
        saveauc['prc_pa_only'] = per_label_prc
...
if __name__ == "__main__":
    ...
    parser.add_argument('--test-only-pa', action='store_true')
