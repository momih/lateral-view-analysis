...
orion -v hunt --config orion_config.yaml ./hyperparam_search.py 'CLUSTER' ~/projects/rpp-bengioy/jpcohen/PADCHEST_SJ/labels_csv/joint_PA_L.csv ~/projects/rpp-bengioy/jpcohen/PADCHEST_SJ/labels_csv/splits_PA_L_666.pkl /lustre04/scratch/cohenjos/PC-output/hadrien-test-will-be-removed --exp_name {trial.id} --seed 666 --epochs 20 --model-type 'dualnet' --target 'joint' --batch_size 8 --learning_rate 'orion~loguniform(1e-5, 1.0)' --dropout 'orion~uniform(0, 5, discrete=True)' --optim "orion~choices(['adam', 'sgd'])" --reduce_period 'orion~uniform(5, 30, discrete=True)' --gamma 'orion~uniform(0.1, 0.5)' --log '{exp.working_dir}/{exp.name}_{trial.id}/exp.log'
